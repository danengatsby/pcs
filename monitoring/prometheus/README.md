...
```bash
cp monitoring/prometheus/prometheus.pcp.example.yml /etc/prometheus/prometheus.yml
```

2. Update `targets` from `api.example.org` to production PCP API host.
...
```bash
cp monitoring/prometheus/secrets/pcp_metrics_bearer_token.example monitoring/prometheus/secrets/pcp_metrics_bearer_token
chmod 600 monitoring/prometheus/secrets/pcp_metrics_bearer_token
```
...
6. Dashboard is auto-provisioned as `PCP / PCP API Overview`.

## 3) Install bearer token for Prometheus

Write the same `METRICS_BEARER_TOKEN` value in:

`/etc/prometheus/secrets/pcp_metrics_bearer_token`

Recommended permissions:

```bash
chmod 600 /etc/prometheus/secrets/pcp_metrics_bearer_token
chown prometheus:prometheus /etc/prometheus/secrets/pcp_metrics_bearer_token
```

## 4) Install alert rules

```bash
cp monitoring/prometheus/alerts.pcp.yml /etc/prometheus/rules/alerts.pcp.yml
```
...
The 7-day cut-over is implemented as `10080` 1-minute samples in `alerts.pcp.yml`.
...
```promql
# p95 latency (5m), same filter as alerts.pcp.yml
histogram_quantile(
  0.95,
  sum by (le) (
    rate(pcp_http_request_duration_seconds_bucket{route!="/api/metrics",route!="/api/health"}[5m])
  )
)
```
...
pcp:api_latency_seconds:p95_5m
```
