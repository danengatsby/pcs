...
test("domain modules should expose enterprise list endpoints", async () => {
  const [finance, organizations, elections, journal] = await Promise.all([
    ...
    request(app).get("/api/governance/journal?limit=5").expect(200),
  ]);
  ...
  assert.equal(typeof journal.body?.meta?.total, "number");
});
