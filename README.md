...
Formularul de aderare si buildul nu necesita chei sau servicii CAPTCHA externe. Protectia locala pastreaza limitarea cererilor, campul-capcana `website`, validarea datelor si prevenirea inscrierilor duplicate.
...
- `npm run build:client:production` construieste frontend-ul fara sa citeasca secretele din `server/.env`
...
E2E-ul full-stack pentru aderare verifica raspunsul `201`, inregistrarea din baza de test si absenta cererilor catre servicii externe de verificare. Necesita `TEST_DATABASE_URL` explicit.
